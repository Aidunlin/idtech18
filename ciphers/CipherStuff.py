"""
Welcome to CipherStuff, created by Aidunlin. I have chosen to insert all code within functions so that it can be run as
many times as you want without restarting the program. There are ten ciphers to choose from, and each of them can both
encipher and decipher text. Deciphered text is called 'plaintext', and enciphered text is called 'ciphertext'. Depending
on the cipher, you may need to adjust your text according to how the program works. For instance, Morse Code will only
take periods (.) and dashes (-) as dots/dashes. Have fun with this program, and do as you will with it.
"""
...
def autokey():
    ...
    if text_choice == "p":
        print("Input the plaintext:")
        plain_text = input().lower()
        extended_keyword = user_keyword + plain_text
        count = 0
        for character in plain_text:
            if character != " ":
                cipher_text += alphabet_table[alphabet.find(character)][alphabet.find(extended_keyword[count])]
    ...
    if text_choice == "c":
        print("Input the ciphertext:")
        cipher_text = input().lower()
        extended_keyword = user_keyword + cipher_text
        count = 0
        for character in cipher_text:
            if character != " ":
                plain_text += alphabet[alphabet_table[alphabet.find(extended_keyword[count])].index(character)]
...
def tap():
        ...
        for character in plain_text:
            if character != " ":
                for row in alphabet_table:
                    if character in row:
                        cipher_text += ("." * (alphabet_table.index(row) + 1)) + " " + (
                                "." * (row.index(character) + 1)) + "  "
            if character == " ":
                cipher_text += "/  "
    ...
    if text_choice == "c":
        ...
        for index in range(int(len(cipher_numbers) / 2)):
            cipher_number_groups.append(str(cipher_numbers.index(str(index * 2))) +
                                        str(cipher_numbers.index(str(index * 2 + 1))))
        for group in cipher_number_groups:
            plain_text += alphabet_table[int(group[0]) + 1][int(group[1]) + 1]
